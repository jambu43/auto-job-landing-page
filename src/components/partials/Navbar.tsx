...
export default function Navabar() {
  ...
  return (
    <Box bg={useColorModeValue("#fff", "#fff")}>
      <Container maxW={"7xl"}>
        <Flex
        ...
        >
          ...
          <Flex flex={{ base: 1 }} justify={{ base: "center", md: "start" }}>
            <NavLink to="/">
              <img src="/images/logos/logo.svg" width="200px" />
            </NavLink>
  ...
  return (
    <Stack direction={"row"} spacing={4}>
      {NAV_ITEMS.map((navItem) => (
        <Box key={navItem.label} style={{ marginLeft: 20, marginTop: 10 }}>
          <Popover trigger={"hover"} placement={"bottom-start"}>
            <PopoverTrigger>
              <NavLink
                to={navItem.to ?? "#"}
                style={{ margin: "0 20px", color: "#000" }}
              >
                {navItem.label}
              </NavLink>
            </PopoverTrigger>
